...
st.markdown("""
    <style>
    /* 1. SEITENABSTAND GLOBAL */
    .block-container { 
        padding-top: 2.0rem !important; 
        padding-bottom: 0rem !important;
    }
    
    /* 2. STARTSEITE: ÜBERSCHRIFT (Kompakt & Kontrastreich) */
    .main-header {
    ...
    /* 3. STARTBILD: Begrenzung der Höhe, damit der Button immer sichtbar ist */
    [data-testid="stImage"] > img {
        max-height: 220px !important; /* Begrenzt die vertikale Ausdehnung */
...
if 'cal_key' not in st.session_state: st.session_state.cal_key = 0 # Für Force-Refresh
...
if not st.session_state.started:
    ...
    # Kein st.write("---") mehr, um Platz zu sparen
...
# --- 2. HAUPT-APP ---
    ...
    # Kalender mit dynamischem Key für Force-Refresh
    state = calendar(events=calendar_events + holidays, options=cal_options, key=f"cal_{st.session_state.cal_key}")
    ...
    if st.session_state.selected_date:
        ...
        with st.form("quick_new_form"):
            ...
            if c1.form_submit_button("Speichern"):
                ...
                st.session_state.selected_date = None
                st.session_state.cal_key += 1 # Force refresh
                st.rerun()
        ...
        try:
            ...
            with st.form("edit_form"):
                ...
                if c2.form_submit_button("🗑️ Löschen"):
                    ...
                    st.session_state.edit_id = None
                    st.session_state.cal_key += 1 # Key ändern erzwingt komplette Neu-Zeichnung des Kalenders
                    st.toast("Gelöscht!")
                    # 3. Kleiner Moment warten und Rerun
                    time.sleep(0.2)
                    st.rerun()
