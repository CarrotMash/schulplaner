...
# --- 2. KLAUSUREN ---
    ...
    cal_ev = []
    for d in k_data:
        cal_ev.append({"id": str(d["id"]), "title": d["titel"], "start": d["start_date"], "backgroundColor": d["color"], "allDay": True, "textColor": "white"})
...
# --- 3. STUNDENPLÄNE ---
    ...
    st.markdown(f'<div class="main-header">Schulplaner Pläne</div>', unsafe_allow_html=True)
    c_cols = st.columns(3)
    ...
    for i, day in enumerate(disp_days):
        with cols[i]:
            st.markdown(f"<div class='day-header' style='background:{c_color};'>{day}</div>", unsafe_allow_html=True)
