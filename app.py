...
if 'cal_key' not in st.session_state: st.session_state.cal_key = "vFinal_1"
...
# --- 4. BUS-CHECK (VERIFIZIERTE LINKS) ---
elif st.session_state.view == 'bus':
    st.markdown('<p class="main-header">Bus-Check</p>', unsafe_allow_html=True)
    st.info("Wähle eine Haltestelle, um den offiziellen NAH.SH Live-Monitor zu öffnen.")
    
    # Haltestellen-Konfiguration mit Such-Parametern (robustestes System)
    # 1. Seefischmarkt
    ...
    st.link_button("➔ Monitor öffnen", "https://www.nah.sh/de/fahrplan/abfahrtsmonitor/?stop=Kiel%2C+Seefischmarkt", use_container_width=True)
    
    # 2. Amboßweg
    st.markdown("<div class='bus-station-card'>Amboßweg<br><small>Zuhause ➔ Schule (Ri. Kiel)</small></div>", unsafe_allow_html=True)
    st.link_button("➔ Monitor öffnen", "https://www.nah.sh/de/fahrplan/abfahrtsmonitor/?stop=Sch%C3%B6nkirchen%2C+Ambo%C3%9Fweg", use_container_width=True)
    
    # 3. Linas Diek
    st.markdown("<div class='bus-station-card'>Linas Diek<br><small>Zuhause ➔ Schule (Ri. Kiel)</small></div>", unsafe_allow_html=True)
    st.link_button("➔ Monitor öffnen", "https://www.nah.sh/de/fahrplan/abfahrtsmonitor/?stop=Sch%C3%B6nkirchen%2C+Linas+Diek", use_container_width=True)
